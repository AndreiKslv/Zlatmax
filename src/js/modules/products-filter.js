...
function productsFilter() {
	...
	const filterSelectForm = document.querySelector('.select__form')
	const filterSelectInput = filterSelectForm.querySelector('.select__input')

	const showMoreButton = document.querySelector('.filter__showmore-button')

	const pagination = document.querySelector('.pagination')
	const paginationUl = pagination.querySelector('.pagination__list')
	...
	let limitedProductsAtStart
	let filteredProducts
	let filteredProductsLimited
	...
	let currentPage = 1
	let pagesCount
	let showMoreProducts = 0

	limitedProductsAtStart = productsData.slice(pageStart, pageEnd)
	filteredProducts = [...productsData]
	
	// Показать начальные товары (21 штука)
	renderProducts(limitedProductsAtStart)
	...
	filterAside.addEventListener('change', function(e) {
		const checkboxFilter = e.target.classList.contains('filter__checkbox')
		const isCheckboxFilterAll = e.target.hasAttribute('data-filter-all')
	...
	pagination.addEventListener('click', (e) => {
		...
		if (e.target.hasAttribute('page') && !e.target.classList.contains('pagination__list-item--active')) {
			const pageNumber = Number(e.target.getAttribute('page'))
	...
	function checkDisableButton() {
		currentPage !== 1 ? buttonPrev.removeAttribute('disabled') : buttonPrev.setAttribute('disabled', '')
		currentPage !== pagesCount ? buttonNext.removeAttribute('disabled') : buttonNext.setAttribute('disabled', '')
	}
	...
	function renderProducts(arr) {
		...
		arr.forEach(function (item) {
			...
			filterGrid.insertAdjacentHTML('beforeend', productHTML)

			starRating()
		})
	}

	function filterProducts(start, end) {
		const productionChecked = filterAside.querySelectorAll('[data-filter-production]:checked')
		const steelChecked = filterAside.querySelectorAll('[data-filter-steel]:checked')
		const hiltChecked = filterAside.querySelectorAll('[data-filter-hilt]:checked')
		const pommelChecked = filterAside.querySelectorAll('[data-filter-pommel]:checked')
		const gildingChecked = filterAside.querySelectorAll('[data-filter-gilding]:checked')
		const ratingChecked = filterAside.querySelectorAll('[data-filter-rating]:checked')
		const allChecked = [productionChecked, steelChecked, hiltChecked, pommelChecked, gildingChecked, ratingChecked]
		const checkedArr = []

		const priceMin = parseInt(filterAside.querySelector('#range-slider-price-1').value)
		const priceMax = parseInt(filterAside.querySelector('#range-slider-price-2').value)
		const totalLengthMin = parseInt(filterAside.querySelector('#range-slider-totalLength-1').value)
		const totalLengthMax = parseInt(filterAside.querySelector('#range-slider-totalLength-2').value)
		const bladeLengthMin = parseInt(filterAside.querySelector('#range-slider-bladeLength-1').value)
		const bladeLengthMax = parseInt(filterAside.querySelector('#range-slider-bladeLength-2').value)
		const bladeWidthMin = parseInt(filterAside.querySelector('#range-slider-bladeWidth-1').value)
		const bladeWidthMax = parseInt(filterAside.querySelector('#range-slider-bladeWidth-2').value)

		allChecked.forEach(function(item, index) {
			checkedArr.push([])
			item.forEach(function(item) {
				const value = item.getAttribute(`${item.attributes[3].name}`)
				checkedArr[index].push(value)
			})
		})
	
		filteredProducts = productsData.filter(item => 
			(item.price >= priceMin && item.price <= priceMax) &&
			(item.totalLength >= totalLengthMin && item.totalLength <= totalLengthMax) &&
			(item.bladeLength >= bladeLengthMin && item.bladeLength <= bladeLengthMax) && 
			(item.bladeWidth >= bladeWidthMin && item.bladeWidth <= bladeWidthMax) && 
			(checkedArr[0].includes(item.production) && checkedArr[1].includes(item.steel) && 
	...
	function setAllNotChecked(e) {
		const checkboxes = e.target.closest('.filter__checkbox-list').querySelectorAll('.filter__checkbox')
		checkboxes.forEach(item => item.checked = false)
	}

	function setAllChecked(e) {
		const checkboxes = e.target.closest('.filter__checkbox-list').querySelectorAll('.filter__checkbox')
